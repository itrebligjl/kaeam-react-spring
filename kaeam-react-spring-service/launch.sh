...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "kaeam-react-spring-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name kaeam-react-spring-service apps/kaeam-react-spring-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app kaeam-react-spring-service:1.0-SNAPSHOT
    oc expose svc/kaeam-react-spring-service
else

	echo "Launching the application locally..."
	pattern="kaeam-react-spring-service"
	files=( $pattern )
	cd ${files[0]}
	executable="$(ls  *target/*.jar | tail -n1)"
	java --add-modules java.se.ee -jar "$executable"
fi
