...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "kaeam-react-spring-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name kaeam-react-spring-service apps/kaeam-react-spring-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app kaeam-react-spring-service:1.0-SNAPSHOT
		call oc expose svc/kaeam-react-spring-service
	) else (
		echo "Launching the application locally..."
		cd kaeam-react-spring-service
		cd target
